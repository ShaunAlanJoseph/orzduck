...
class TicTacDuel:
    ...
    async def create_duel(
        cls, player1: int, player2: int, problems_loaded: List["CFProblem"], rating: int
    ):
        duel_data: Dict[str, Any] = {
            "duel_id": generate_string(8),
            "player1": player1,
            "player2": player2,
            "start_time": get_time(),
            "status": "ongoing",
    ...
    def __init__(self, duel_data: Dict[str, Any]):
        ...
        self.end_time: Optional[int] = duel_data.get("end_time")
    ...
    async def refresh_duel(self):
        ...
        timeline: List[Tuple[int, "CFProblem", int, int]] = []  # time, problem, idx, player
...
class TickTacDuelView(BaseView):
    ...
    def _add_items(self):
        ...
        if self.duel.status != "finished":
            self._add_button(label="REFRESH", custom_id="refresh", row=1)
